...
"""
    ...
    async def process_uploaded_file(self, uploaded_file, delay_seconds: float = 0) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            llm_response = await self._invoke_llm_with_retry(formatted_prompt)
    ...
    def process_uploaded_file_sync(self, uploaded_file, delay_seconds: float = 0) -> Dict[str, Any]:
        """
        Synchronous wrapper for async process_uploaded_file
        """
        return asyncio.run(self.process_uploaded_file(uploaded_file, delay_seconds))
