...
class InvoiceImageProcessor:
    ...
    async def _invoke_gemini_with_retry(self, content) -> Any:
        """
        ...
        for attempt in range(max_retries):
            try:
                # Run the synchronous generate_content in executor
                loop = asyncio.get_event_loop()
                response = await loop.run_in_executor(None, self.model.generate_content, content)
                return response
            except Exception as e:
                err_msg = str(e)
                if '429' in err_msg and 'retry_delay' in err_msg:
                    ...
                    if match:
                        ...
                        logger.warning(f"Gemini API rate limit hit (429). Attempt {attempt + 1}/{max_retries}. Retrying after {retry_delay} seconds...")
    ...
    async def extract_structured_data_with_gemini(self, plain_text: str, key_value_pairs: Dict[str, str], 
        ...
        try:
            ...
            response = await self._invoke_gemini_with_retry(content)
    ...
    def process_uploaded_file_sync(self, uploaded_file) -> Dict[str, Any]:
        """
        Synchronous wrapper for async process_uploaded_file
        """
        return asyncio.run(self.process_uploaded_file(uploaded_file))

    async def process_file_path(self, file_path: str) -> Dict[str, Any]:
        """
    ...
    async def extract_structured_data_with_gemini_legacy(self, ocr_text: str, max_retries: int = 2) -> Dict[str, Any]:
