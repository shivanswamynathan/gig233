...
class ItemWiseReconciliationProcessor:
    ...
    async def _create_item_reconciliation_record(self, invoice_item: InvoiceItemData, match_evaluation: Dict[str, Any]) -> 'InvoiceItemReconciliation':
        ...
        reconciliation_data = {
            ...
            'amount_match_score': Decimal('1.0000') if safe_tolerance_extract(variances.get('amount_variance', {})) else Decimal('0.0000'),
            'quantity_match_score': Decimal('1.0000') if safe_tolerance_extract(variances.get('quantity_variance', {})) else Decimal('0.0000'),
