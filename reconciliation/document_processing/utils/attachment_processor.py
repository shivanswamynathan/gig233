    ...
    def _create_invoice_items(self, invoice_data: InvoiceData, line_items: List[Dict[str, Any]], attachment_info: Dict[str, Any]):
        ...
        try:
            ...
            for idx, item in enumerate(line_items, 1):
                ...
                item_value_str = (item.get('taxable_amount', '') or item.get('gross_amount', '') )

                if not item_value_str and item_record.quantity and item_record.unit_price:
                    calculated_value = item_record.quantity * item_record.unit_price
                    item_value_str = str(calculated_value)
                    logger.info(f"Calculated taxable amount for item {idx}: {calculated_value}")
