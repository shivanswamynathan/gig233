...
class SmartFileClassifier:
    ...
    @staticmethod
    async def download_and_analyze(url: str) -> Dict[str, Any]:
        """
        Download file and perform complete analysis with proper URL encoding (ASYNC VERSION)
        ...
        try:
            ...
            if detected_format == 'PDF':
                # Run blocking PDF analysis in thread pool
                can_extract_text = await loop.run_in_executor(None, SmartFileClassifier._analyze_pdf_content, result['temp_file_path'])
                if can_extract_text:
                ...
                # Run blocking image verification in thread pool
                is_valid_image = await loop.run_in_executor(None, SmartFileClassifier._verify_image, result['temp_file_path'])
                if is_valid_image:
