...
class AttachmentProcessorFromGrn:
    ...
    async def _download_and_analyze_async(self, url: str) -> Dict[str, Any]:
        """Use OUR file classifier (now async)"""
        return await self.file_classifier.download_and_analyze(url)
    
    async def _process_pdf_text_async(self, temp_file_path: str) -> Dict[str, Any]:
        """Direct async version of PDF text processing with LLM"""
        return await self.text_pdf_processor.process_file_path(temp_file_path)
    
    async def _process_pdf_image_async(self, temp_file_path: str) -> Dict[str, Any]:
        """Direct async version of PDF image processing with OCR + LLM"""
        return await self.image_processor.process_file_path(temp_file_path)
    
    async def _process_image_async(self, temp_file_path: str) -> Dict[str, Any]:
        """Direct async version of image processing with OCR + LLM"""
        return await self.image_processor.process_file_path(temp_file_path)
    ...
    async def _mark_grn_records_as_processed(self, url: str, row_number: int = None):
        ...
        try:
            ...
            updated_count = await sync_to_async(qs.update)(extracted_data=True)
        ...
        try:
            ...
            for idx, item in enumerate(line_items, 1):
                ...
                numeric_fields = {
                    ...
                    'invoice_value_item_wise': lambda item: (item.get('taxable_amount', '') or item.get('gross_amount', '')),
                    'cgst_rate': 'cgst_rate',
