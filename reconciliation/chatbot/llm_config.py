...
class LLMConfig:
    ...
    def _determine_analysis_type(self, question: str) -> str:
        ...
        analysis_keywords = {
            'mismatch_analysis': ['mismatch', 'not match', 'differ', 'discrepancy', 'why not matching'],
            'variance_analysis': ['variance', 'difference', 'amount diff', 'price diff', 'quantity diff'],
            'exception_analysis': ['exception', 'error', 'issue', 'problem', 'critical', 'urgent'],
            'workflow_analysis': ['workflow', 'approval', 'pending', 'process', 'efficiency', 'bottleneck'],
            'trend_analysis': ['trend', 'over time', 'monthly', 'weekly', 'pattern', 'growth']
        }
